 ...
 * Search and explore the Yu-Gi-Oh! card database
 */
...
async function getRandomCard() {
  // First get all cards with IDs
  const allCardsUrl = 'https://db.ygoprodeck.com/api/v7/cardinfo.php?misc=yes';
...
`);
  ...
  try {
    ...
    if (options.random) {
      ...
      // Filter results (API doesn't support all filters)
      if (options.price && cards.data) {
        // Fetch full card info for prices
        const cardIds = cards.data.map(c => c.id).join(',');
        const priceUrl = `https://db.ygoprodeck.com/api/v7/cardinfo.php?id=${cardIds}`;
        const priceRes = await fetch(priceUrl);
        cards = await priceRes.json();
      }
    }
    ...
    // Show results
    const cardsToShow = options.random ? cards.data : cards.data.slice(0, 10);
    ...
    const totalCards = cards.data?.length || cards.length || 0;
    if (totalCards > 10 && !options.random) {
      console.log(`\n${COLORS.dim}... and ${totalCards - 10} more cards${COLORS.reset}`);
    }
